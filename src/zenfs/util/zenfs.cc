...
DEFINE_string(zenfs_path, "", "File path to file in zenfs");

namespace ROCKSDB_NAMESPACE {

HybridBlockDevice *hbd_open(bool readonly) {
  std::vector<std::string> bdevnames_vector;
  std::string delimiter = "_", token, str = FLAGS_zbd;
  size_t pos = 0;
  ...
  HybridBlockDevice *hbd; 

  hbd = new HybridBlockDevice(bdevnames_vector, nullptr);
  IOStatus open_status = hbd->Open(readonly);

  if (!open_status.ok()) {
    fprintf(stderr, "Failed to open fused zoned block device: %s, error: %s\n",
...
Status zenfs_mount(HybridBlockDevice *hbd, ZenFS **zenFS, bool readonly) {
  fprintf(stderr, "DEBUG: zenfs_mount()\n");
  Status s;

  *zenFS = new ZenFS(hbd, FileSystem::Default(), nullptr);
...
int zenfs_tool_mkfs() {
  ...
  HybridBlockDevice *hbd = hbd_open(false);
  if (hbd == nullptr) return 1;

  ZenFS *zenFS;
  s = zenfs_mount(hbd, &zenFS, false);
  ...
  fprintf(stdout, "INFO: For (Fused) ZBD %s, device scheduler is set to mq-deadline.\n",
          FLAGS_zbd.c_str());
  fprintf(stdout, "ZenFS file system created. Free space: %lu MB\n",
          hbd->GetFreeSpace() / (1024 * 1024));
...
void list_children(ZenFS *zenFS, std::string path) {
  ...
  fprintf(stderr, "DEBUG: list_children() path = %s\n", path.c_str());
  if (!io_status.ok()) {
    fprintf(stderr, "DEBUG: list_children() GetChildren failed\n");
  }
...
int zenfs_tool_list() {
  Status s;
  HybridBlockDevice *hbd = hbd_open(true);
  if (hbd == nullptr) return 1;

  ZenFS *zenFS;
  s = zenfs_mount(hbd, &zenFS, true);
  if (!s.ok()) {
...
int zenfs_tool_df() {
  ...
  uint64_t used = hbd->GetUsedSpace();
  uint64_t free = hbd->GetFreeSpace();
  uint64_t reclaimable = hbd->GetReclaimableSpace();
...
IOStatus zenfs_tool_copy_dir(FileSystem *f_fs, std::string f_dir, FileSystem *t_fs, std::string t_dir) {
  ...
  fprintf(stderr, "DEBUG: zenfs_tool_copy_dir(): %s %s\n", f_dir.c_str(), t_dir.c_str());
...
int zenfs_tool_backup() {
  ...
  status = zenfs_mount(hbd, &zenFS, false);
  if (!status.ok()) {
    fprintf(stderr, "Failed to mount filesystem, error: %s\n",
...
int zenfs_tool_backup_file() {
  ...
  io_status = zenfs_tool_copy_file(zenFS, FLAGS_zenfs_path, FileSystem::Default().get(), FLAGS_path);
  if (!io_status.ok()) {
    fprintf(stderr, "Copy failed, error: %s\n", io_status.ToString().c_str());
    return 1;
  }
...
int main(int argc, char **argv) {
  ...
  } else if (subcmd == "backup_file") {
    return ROCKSDB_NAMESPACE::zenfs_tool_backup_file();
