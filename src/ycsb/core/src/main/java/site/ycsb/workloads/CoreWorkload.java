...
public class CoreWorkload extends Workload {
  ...
  protected TraceReader tracereader = null;
  ...
  // added for H2Zone
  private double readZipf() {
    File file = new File("SETTINGS_DIR/zipf");
    ...
    try {
      ...
      while ((text = reader.readLine()) != null) {
        zipfianconstant = Double.parseDouble(text);
        System.out.println("zipfianconstant " + zipfianconstant + " read");
      }
  ...
  private double readZetan() {
    File file = new File("SETTINGS_DIR/zeta");
    BufferedReader reader = null;
    double zeta = 26.4690282;
    ...
    try {
      ...
      while ((text = reader.readLine()) != null) {
        zeta = Double.parseDouble(text);
        System.out.println("zeta " + zeta + " read");
      }
    } catch (FileNotFoundException e) {
      e.printStackTrace();
    } catch (IOException e) {
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    if (p.getProperty(INSERT_ORDER_PROPERTY, INSERT_ORDER_PROPERTY_DEFAULT).compareTo("hashed") == 0) {
      ...
      System.out.println("orderedinserts = false");
    } else {
      orderedinserts = true;
      System.out.println("orderedinserts = true");
    }
    ...
    if (requestdistrib.compareTo("uniform") == 0) {
      ...
      double zipfianconstant = readZipf();
      double zeta = readZetan();
      System.out.println("zipfianconstant " + zipfianconstant);
      System.out.println("zeta " + zeta);
      System.out.println("insertstart " + insertstart);
      System.out.println("insertend " + (insertstart + insertcount + expectednewkeys));

      if (zipfianconstant >= 1.0) {
        keychooser = new ReadFromFileGenerator(insertstart, insertstart + insertcount + expectednewkeys);
      } else { 
        //keychooser = new ScrambledZipfianGenerator(insertstart, insertstart + insertcount + expectednewkeys);
        //keychooser = new ScrambledZipfianGenerator(insertstart, insertstart + insertcount + expectednewkeys, 
        //    zipfianconstant);
        keychooser = new ScrambledZipfianGenerator(insertstart, insertstart + insertcount + expectednewkeys, 
    zipfianconstant, zeta);
      }
    ...
    } else if (requestdistrib.equals("trace")) { // Added for H2Zone: twemcache testing
  ...
  public boolean doTransaction(DB db, Object threadstate) {
    ...
    if (tracereader != null) {
      String op = tracereader.getOperation();
      switch (op) {
      case "get": 
        operation = "READ";
        break;
      case "set":
        ...
        break;
      default:
        tracereader.nextRequest();
        if (tracereader.isStopRequested()) {
          requestStop();
        }
        return true;
      }
  ...
  public void doTransactionUpdate(DB db) {
    ...
    if (tracereader == null) {
      ...
      ByteIterator data = new RandomByteIterator(tracereader.getValueLength());
      keyname = tracereader.getKey(); 
      values = new HashMap<>();
      values.put("key", data); // the key in the value
      tracereader.nextRequest();
      if (tracereader.isStopRequested()) {
        requestStop();
      }
  ...
  public void doTransactionInsert(DB db) {
    ...
    try {
      ...
      if (tracereader == null) {
        ...
        ByteIterator data = new RandomByteIterator(tracereader.getValueLength());
        values = new HashMap<>();
        values.put(tracereader.getKey(), data);
        tracereader.nextRequest();
      }
  ...
  protected static DiscreteGenerator createOperationGenerator(final Properties p) {
    ...
    System.out.println("operationchooser read " + readproportion);
    System.out.println("operationchooser update " + updateproportion);
    System.out.println("operationchooser insert " + insertproportion);
    System.out.println("operationchooser scan " + scanproportion);
    System.out.println("operationchooser rmw " + readmodifywriteproportion);
