...
public class RocksDBClient extends DB {
  ...
  // added for H2Zone test
  private boolean updateIsRmw = true;
  ...
  public void init() throws DBException {
    synchronized(RocksDBClient.class) {
      if(rocksDb == null) {
        ...
        // added for H2Zone test
        File file = new File("SETTINGS_DIR/updateIsRmw");
        BufferedReader reader = null;
        Integer updateIsRmwInt = 0;
        ...
        try {
          ...
          while ((text = reader.readLine()) != null) {
            updateIsRmwInt = Integer.parseInt(text);
            System.out.println("updateIsRmwInt " + updateIsRmwInt + " read");
            if (updateIsRmwInt > 0) {
              updateIsRmw = true;
            } else {
              updateIsRmw = false;
  ...
  private RocksDB initRocksDB() throws IOException, RocksDBException {
    ...
    final int rocksThreads = 12; // Runtime.getRuntime().availableProcessors() * 2;
  ...
  public Status read(final String table, final String key, final Set<String> fields,
    ...
    try {
      ...
      if(values == null) {
        System.out.println("Rocksdb read not found: " + key);
        return Status.NOT_FOUND;
      }
      deserializeValues(values, fields, result);
      return Status.OK;
    } catch(final RocksDBException e) {
      System.out.println("Rocksdb read error: " + key + " " + e.getMessage());
