...
struct IOOptions {
  ...
  IOOptions() : timeout(0), prio(IOPriority::kIOLow), 
    type(IOType::kUnknown), orig_offset(1ull << 63),
    orig_length(1ull << 63) {}
};
...
#undef GetFreeSpace
  ...
  virtual void UpdateLevel(uint64_t /*file_id*/, int, int /*level*/) {}
...
class FSRandomAccessFile {
 public:
  ...
  virtual IOStatus InformCacheRecords(Cache::KeyRecords& /*key_records*/) {
    return IOStatus::NotSupported("InformCacheRecords not supported.");
  }

  virtual void CompactionHint(const CompactionHints& /*hint*/) {} 
