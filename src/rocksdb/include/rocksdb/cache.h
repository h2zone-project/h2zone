...
class Cache {
 public:
  ...
  // added for H2Zone
  enum KeyRecord : uint8_t {
    kNotSet = 0,
    kInsert = 1,
    kEvictDueToInsert = 2,
    kImmediateEvict = 3,
    kRefCountZero = 4,
    kSetCapacity = 5,  // May not be used  
    kErase = 6,    // No need to save in ghost cache 
    kRelease = 7,  // No need to save in ghost cache 
  };

  struct KeyRecordUnit {
    ...
    KeyRecordUnit() : record(kNotSet) {} 
    KeyRecordUnit(uint32_t _file_number, uint32_t _offset, uint32_t _length, 
        char* _content, KeyRecord _record) : 
      file_number(_file_number), offset(_offset), length(_length),  
      content(_content), record(_record) {} 
  };

  struct KeyRecords {
    std::vector<KeyRecordUnit> target;
    KeyRecords() { target.clear(); }

    void Clear() { target.clear(); }
    void Push(KeyRecordUnit key_record) { target.push_back(key_record); }
    void PushAll(KeyRecords key_records) {
  ...
  virtual void SetNeedsToSaveRecords(bool /*need*/) {}
  virtual KeyRecords ReleaseKeyRecords() { return KeyRecords(); } 
