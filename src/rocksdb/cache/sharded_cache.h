...
class CacheShard {
 public:
  ...
  // added for H2Zone
  virtual void PrepareSingleKeyRecord(const Slice& /**/, 
      void*, Cache::KeyRecord /**/) {}
  virtual Cache::KeyRecords ReleaseKeyRecords() { return Cache::KeyRecords(); }
  virtual void ClearKeyRecords() {}
  virtual void SetNeedsToSaveRecords(bool param) { needs_to_save_records_ = param; }

 protected:
  CacheMetadataChargePolicy metadata_charge_policy_ = kDontChargeCacheMetadata;
  std::mutex mtx_;
  bool needs_to_save_records_ = true;
};
...
class ShardedCache : public Cache {
 public:
  ...
  void PrepareKeyRecordsFromShard(CacheShard* cache_shard); 
  virtual KeyRecords ReleaseKeyRecords() override;
  virtual void SetNeedsToSaveRecords(bool need) override { 
    needs_to_save_records_ = need; 
  }
 ...
 private:
  ...
  bool needs_to_save_records_ = false;
  std::mutex mtx_;
  KeyRecords key_records_;
};
