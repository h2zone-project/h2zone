...
Status ShardedCache::Insert(const Slice& key, void* value, size_t charge,
  ...
  CacheShard* cache_shard = GetShard(Shard(hash));
  Status s = cache_shard 
    ->Insert(key, hash, value, charge, deleter, handle, priority);
  PrepareKeyRecordsFromShard(cache_shard); 
  return s;
}
...
Cache::Handle* ShardedCache::Lookup(const Slice& key, Statistics* /*stats*/) {
  uint32_t hash = HashSlice(key);
  CacheShard* cache_shard = GetShard(Shard(hash));
  Cache::Handle* s = cache_shard->Lookup(key, hash);
  PrepareKeyRecordsFromShard(cache_shard); 
  return s;
}
...
bool ShardedCache::Release(Handle* handle, bool force_erase) {
  uint32_t hash = GetHash(handle);
  CacheShard* cache_shard = GetShard(Shard(hash));
  bool s = cache_shard->Release(handle, force_erase);
  PrepareKeyRecordsFromShard(cache_shard); 
  return s;
}
...
bool ShardedCache::Release(Handle* handle, bool useful, bool force_erase) {
  uint32_t hash = GetHash(handle);
  CacheShard* cache_shard = GetShard(Shard(hash));
  bool s = cache_shard->Release(handle, useful, force_erase);
  PrepareKeyRecordsFromShard(cache_shard); 
  return s;
}
...
void ShardedCache::PrepareKeyRecordsFromShard(CacheShard* cache_shard) {
  mtx_.lock();

  if (needs_to_save_records_) {
    KeyRecords ret = cache_shard->ReleaseKeyRecords();

    for (auto it : ret.target) {
      key_records_.target.push_back(it);
    }
    if (key_records_.target.size() > 10) {
      for (auto it : key_records_.target) {
        if (it.content) {
          delete[] it.content;
        }
      }
      key_records_.target.clear();
    }

  } else {
    cache_shard->SetNeedsToSaveRecords(false);
    cache_shard->ClearKeyRecords();
  }
  mtx_.unlock();
}

Cache::KeyRecords ShardedCache::ReleaseKeyRecords() {
  mtx_.lock();
  KeyRecords ret;
  ret = key_records_;
  key_records_.Clear();
