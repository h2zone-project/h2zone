...
Status LRUCacheShard::InsertItem(LRUHandle* e, Cache::Handle** handle,
...

  {
    ...
    if (needs_to_save_records_) {
      for (auto& it : last_reference_list) {
        PrepareSingleKeyRecord(it->key(), it->value, 
            Cache::KeyRecord::kEvictDueToInsert);
      }
    }
...
void LRUCacheShard::PrepareSingleKeyRecord(
    const Slice& key, void* value, Cache::KeyRecord key_record) {

  Slice slice = key;
  uint64_t magic_number;
  GetVarint64(&slice, &magic_number);

  if (magic_number != 0x40506070) {
    return;
  }

  uint64_t file_type, offset, file_id; 
  GetVarint64(&slice, &file_type);
  if (file_type != kTableFile) {
    return;
  }

  GetVarint64(&slice, &file_id);
  GetVarint64(&slice, &offset);

  Block* block = nullptr;

  block = reinterpret_cast<Block*>(value);

  uint32_t length;
  char* data; 
  
  length = (uint32_t)block->size();
  data = new char[length];
  memcpy(data, block->data(), length);

  mtx_.lock();
  key_records_.Push(Cache::KeyRecordUnit(file_id, offset, length, data, key_record));
  mtx_.unlock();
}

Cache::KeyRecords LRUCacheShard::ReleaseKeyRecords() {
  mtx_.lock();
  Cache::KeyRecords ret;
  for (auto it : key_records_.target) {
    ret.Push(Cache::KeyRecordUnit(it.file_number, it.offset, it.length, 
          it.content, it.record));
  }
  key_records_.Clear();
  mtx_.unlock();
...
void LRUCacheShard::ClearKeyRecords() {
  mtx_.lock();
  for (auto it : key_records_.target) {
    if (it.content) {
      delete[] it.content;
