...
ColumnFamilyOptions* ColumnFamilyOptions::OptimizeLevelStyleCompaction(
  ...
  write_buffer_size = static_cast<size_t>(memtable_memory_budget);
  ...
  target_file_size_base = 1060320051; 

  // make Level1 size equal to Level0 size, so that L0->L1 compactions are fast
  max_bytes_for_level_base = memtable_memory_budget * 2;

  fprintf(stderr, "write_buffer_size %ld\n", write_buffer_size);
  fprintf(stderr, "min_write_buffer_number_to_merge %ld\n", 
      min_write_buffer_number_to_merge);
  fprintf(stderr, "max_write_buffer_number %ld\n", 
      max_write_buffer_number);
  fprintf(stderr, "level0_file_num_compaction_trigger %ld\n", 
      level0_file_num_compaction_trigger);
  fprintf(stderr, "target_file_size_base %ld\n", 
      target_file_size_base);
