...
std::string jni_zenfs_url() {
  FILE *f = nullptr;

  std::string fn = "SETTINGS_DIR/jni_zenfs_url";
  
  f = fopen(fn.c_str(), "r");
  char s[200];
  if (fscanf(f, "%s", s)) {
    fprintf(stderr, "jni read url: %s\n", s);
    if (strcmp(s, "null") == 0) {
      return "";
    }
    return s;
  } 
  return "zenfs://dev:nvme0n1";
}
...
jlong rocksdb_open_helper(JNIEnv* env, jlong jopt_handle, jstring jdb_path,
  ...
  // added for zenfs
  std::string url = jni_zenfs_url();

  if (url != "") {
    static std::shared_ptr<ROCKSDB_NAMESPACE::Env> env_guard;
    ROCKSDB_NAMESPACE::Env* env2;
    ROCKSDB_NAMESPACE::Env::CreateFromUri(ROCKSDB_NAMESPACE::ConfigOptions(),
        "", url.c_str(), &env2, &env_guard);
    opt->env = env2;
  }
  ...
  fprintf(stderr, "rocksdb_open_helper test\n");
...
jlongArray rocksdb_open_helper(
  ...
  fprintf(stderr, "rocksdb_open_helper test 2\n");
  ...
  if (opt->statistics == nullptr) {
    ...
    fprintf(stderr, "opt->statistics created\n");
  }
