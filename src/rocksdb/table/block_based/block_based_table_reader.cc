...
void BlockBasedTable::CompactionHint(const CompactionHints& hint) {
  rep_->file->file()->CompactionHint(hint);
}
...
Status BlockBasedTable::PutDataBlockToCache(
  ...
  if (block_cache != nullptr && block_holder->own_bytes()) {
    ...
    Cache::KeyRecords records = block_cache->ReleaseKeyRecords();
    rep_->file->file()->InformCacheRecords(records);
