  ...
  if (TryGetFromPrefetchBuffer()) {
    ...
    if (io_status_.ok()) {
      if (file_->use_direct_io()) {
        ...
        opts.orig_offset = handle_.offset();
        opts.orig_length = block_size_with_trailer_;

        if (block_size_with_trailer_ > 30000) {
//          fprintf(stderr, "direct: handle_.offset() %ld, "
//              "block_size_with_trailer_ %ld\n", 
